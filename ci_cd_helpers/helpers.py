...
def get_security_wrapping_workflow_name(repo: str, branch: str=None, suffix: str=None) -> str:
    """Get security wrapping workflow name.
    
    This workflow wraps the DLT pipeline.
    
    Example: for the reference pipeline with branch "feature/my-new-feature",
    the wrapping workflow name will be
    "sede-x-enterprise-data-platform-reference-pipeline-feature-my-new-feature-security-workflow".
    
    :param repo: GitHub repository name
    :param branch: branch name
    :param suffix: suffix to add
    :return: security workflow name
    """
    if branch:
        prefix_name = format_resource_prefix(repo, branch)
    else:
        prefix_name = format_repository_name(repo)
    security_wrapping_workflow_name = f"{prefix_name}-security-workflow"

    if suffix:
       security_wrapping_workflow_name = f"{security_wrapping_workflow_name}-{suffix}"

    return security_wrapping_workflow_name
...
def get_optimize_vacuum_wrapping_workflow_name(repo: str, branch: str=None, suffix: str=None) -> str:
    """Get optimize_vacuum wrapping workflow name.
    
    This workflow wraps the DLT pipeline.
    
    Example: for the reference pipeline with branch "feature/my-new-feature",
    the wrapping workflow name will be
    "sede-x-enterprise-data-platform-reference-pipeline-feature-my-new-feature-optimize_vacuum-workflow".
    
    :param repo: GitHub repository name
    :param branch: branch name
    :param suffix: suffix to add
    :return: optimize_vacuum workflow name
    """
    if branch:
        prefix_name = format_resource_prefix(repo, branch)
    else:
        prefix_name = format_repository_name(repo)
    optimize_vacuum_wrapping_workflow_name = f"{prefix_name}-optimize_vacuum-workflow"

    if suffix:
       optimize_vacuum_wrapping_workflow_name = f"{optimize_vacuum_wrapping_workflow_name}-{suffix}"

    return optimize_vacuum_wrapping_workflow_name
